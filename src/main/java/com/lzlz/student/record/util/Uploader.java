...
public class Uploader {
		...
		try {
			...
			while (fii.hasNext()) {
				...
				if (!fis.isFormField()) {
					...
					File file = new File(this.getParentPath(this.url));
	...
	public void uploadBase64(String fieldName) {
		...
		try {
			File outFile = new File(this.getParentPath(this.url));
	...
	private String getFolder(String path) {
		...
		File dir = new File(this.getParentPath(path));
		if (!dir.exists()) {
			try {
				dir.mkdirs();
			} catch (Exception e) {
	...
	private String getParentPath(String path) {
		...
		return (new File(new File(realPath).getParent()).getParent())+ "/resources/" + path;
	}
