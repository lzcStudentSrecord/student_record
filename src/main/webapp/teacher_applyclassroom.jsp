...
<body
	...
	<div
		...
		<div>
			...
			<br>
			<h3 style="text-align: left; margin-left: 25%; color: #000;">
				<strong>使用日期</strong><input name="startdate"
					onfocus="MyCalendar.SetDate(this)" id="startdate"
					style="height: 40px; border: 1px solid #d1d1d1; border-radius: 20px; margin-left: 20px; margin-top: 10px;"
					type="text">
			</h3>
			<br>
			...
			<br>
			<h3 style="text-align: left; margin-left: 25%; color: #000;">
				<strong>归还日期</strong><input name="enddate"
					onfocus="MyCalendar.SetDate(this)" id="enddate"
					style="height: 40px; border: 1px solid #d1d1d1; border-radius: 20px; margin-left: 20px; margin-top: 10px;"
					type="text">
			</h3>
			<br>
			...
			<br> <input class="bootstrap-frm2" type="button" id="subajax" value="提交">
			<br>
		</div>
	</div>
...
<script type="text/javascript">
	$("#subajax").click(
			function() {
				$.ajax({
					url : "/ApplyClassroom/insertByTeacher",
					type : "POST",
					data : {
						reason : $("#reason").val(),
						craddress : $("#craddress").find("option:selected")
								.text(),
						startTime : $("#startdate").val() + ' '
								+ $("#starttime").val().trim() + ":00",
						endTime : $("#enddate").val() + ' '
								+ $("#endtime").val().trim() + ":00"
					},
